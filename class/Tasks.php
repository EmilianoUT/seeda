...
class Tasks {	
	...
	public function insert(){
		...
		if($this->description) {
			...
			INSERT INTO ".$this->tasksTable."(`task_description`, `task_due_date`, `project`, `sales_rep`, `task_status`)
			VALUES(?,?,?,?,?)");
			...
			$stmt->bind_param("ssiis", $this->description, $this->due_date, $this->project, $this->sales_rep, $this->status);
	...
	public function getTasks(){
		if($this->id) {	
			$sqlQuery = "SELECT t.id, t.created, t.task_description, t.task_due_date, t.task_status, p.project_name, u.name, u.id as sales_rep
			FROM ".$this->tasksTable." t 
			LEFT JOIN ".$this->projectTable." p ON t.project = p.id
	...
	public function update(){		
		if($this->id) {				
			...
			SET task_description = ?, task_due_date = ?, project = ?, sales_rep = ?, task_status = ?
			WHERE id = ?"); 			
			$this->description = htmlspecialchars(strip_tags($this->description));			
			$this->due_date = htmlspecialchars(strip_tags($this->due_date));
			$this->project = htmlspecialchars(strip_tags($this->project));
			$this->sales_rep = htmlspecialchars(strip_tags($this->sales_rep));
			$this->status = htmlspecialchars(strip_tags($this->status));				
			$stmt->bind_param("ssiisi", $this->description, $this->due_date, $this->project, $this->sales_rep, $this->status, $this->id);			
			if($stmt->execute()){
				return true;
			}			
		}	
